...
function AcentuacaoPersistence() {

    this.getPersistence = 
    function () {    
          return new Promise(function (resolve, reject) {
              if   (globals.dataBaseType == 1) {
                    var Persistence = require('./relational/acentuacao.js');
                    var persistence = new Persistence();
                    resolve(persistence);
                  }
              else if   (globals.dataBaseType == 2) {
                  /*firebase ainda não possui o model personagem*/
                          var Persistence = require('./firebase/acentuacao.js');          
                          var persistence = new Persistence();
                          resolve(persistence);
                  }
                   else {
...
module.exports = AcentuacaoPersistence;
