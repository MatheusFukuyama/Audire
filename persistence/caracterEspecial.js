...
function CaracterEspecialPersistence() {

    this.getPersistence = 
    function () {    
          return new Promise(function (resolve, reject) {
              if   (globals.dataBaseType == 1) {
                    var Persistence = require('./relational/caracterEspecial.js');
                    var persistence = new Persistence();
                    resolve(persistence);
                  }
              else if   (globals.dataBaseType == 2) {
                  /*firebase ainda não possui o model personagem*/
                          var Persistence = require('./firebase/caracterEspecial.js');          
                          var persistence = new Persistence();
                          resolve(persistence);
                  }
                   else {
...
module.exports = CaracterEspecialPersistence;
