...
function PerguntaContextoPersistence() {

    this.getPersistence = 
    function () {    
          return new Promise(function (resolve, reject) {
              if   (globals.dataBaseType == 1) {
                    var Persistence = require('./relational/perguntaContexto.js');
                    var persistence = new Persistence();
                    resolve(persistence);
                  }
              else if   (globals.dataBaseType == 2) {
                  /*firebase ainda não possui o model personagem*/
                          var Persistence = require('./firebase/perguntaContexto.js');          
                          var persistence = new Persistence();
                          resolve(persistence);
                  }
                   else {
...
module.exports = PerguntaContextoPersistence;
